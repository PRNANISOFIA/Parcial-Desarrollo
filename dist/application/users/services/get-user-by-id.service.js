"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.GetUserByIdService = void 0;
const common_1 = require("@nestjs/common");
const user_id_vo_1 = require("../../../domain/users/value-objects/user-id.vo");
const user_mapper_1 = require("../mapper/user.mapper");
const user_ports_1 = require("../ports/out/user.ports");
let GetUserByIdService = class GetUserByIdService {
    userRepository;
    constructor(userRepository) {
        this.userRepository = userRepository;
    }
    async execute(query) {
        const id = user_id_vo_1.UserId.from(query.id);
        const user = await this.userRepository.findById(id);
        return user ? user_mapper_1.UserMapper.toResponse(user) : null;
    }
};
exports.GetUserByIdService = GetUserByIdService;
exports.GetUserByIdService = GetUserByIdService = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [user_ports_1.UserRepository])
], GetUserByIdService);
//# sourceMappingURL=get-user-by-id.service.js.map